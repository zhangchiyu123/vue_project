1.什么是后台管理系统项目？
前端领域开发的后台管理系统项目，并非是java、php等后台语言

在前面的课程当中，我们已经开发了一个项目：[尚品汇电商平台项目],这个项目主要针对的是用户(游客)，可以让用户在平台当中购买产品，用户购买产品信息从何而来?

比如；前台项目当中的数据来源于卖家，卖家不会数据库的操作，对于卖家，需要把这些产品的信息上传于服务器，写入数据库。
卖家并非程序员，不会数据库的操作(增删改查)，导致卖家，找了一个程序员，开发了一个产品，可以让卖家进行可视化的操作(增删改查)
卖家公司：老板、员工
老板：什么都可以操作[产品的上架和下架、查看员工的个人业绩、其他]
员工：[可能就是查看个人业绩]
...
http://gmall-h5-api.atguigu.cn   新接口

trademark.js
新增： /admin/product/baseTrademark/save  POST
...
// 商品基础属性接口   /admin/product/attrInfoList/{category1Id}/{category2Id}/{category3Id}  GET
// 添加属性与属性值的接口  /admin/product/saveAttrInfo  POST
// 保存属性信息   /admin/product/saveAttrInfo  POST
...
1. 对于某一个组件，如果style添加上scoped属性，给当前子组件的结构中都添加上一个data-v-xxxx自定义属性，会发现vue是通过属性选择器，给需要添加的有元素添加上样式
h3[data-v-7ba5bd90]

2.子组件的根标签(拥有父组件当中的自定义属性:一样的)，如果子组件的根节点，和父组件当中书写的样式相同，也会添加上相应的样式

注意:
如果父组件的样式(scoped)，而且还想影响到子组件的样式，我们可以使用深度选择器
